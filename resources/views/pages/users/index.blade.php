@extends('layouts.customs.app')

@section('title', 'Manage Users')

@push('vendor-style')
    <!-- Vendor css files -->
    <link rel="stylesheet"
        href="{{ asset(mix('vendors/css/tables/datatable/dataTables.bootstrap5.min.css')) }}">
    <link rel="stylesheet"
        href="{{ asset(mix('vendors/css/tables/datatable/responsive.bootstrap5.min.css')) }}">
    <link rel="stylesheet"
        href="{{ asset(mix('vendors/css/tables/datatable/buttons.bootstrap5.min.css')) }}">
    <link rel="stylesheet"
        href="{{ asset(mix('vendors/css/forms/select/select2.min.css')) }}">
    <link rel="stylesheet"
        href="{{ asset(mix('vendors/css/pickers/flatpickr/flatpickr.min.css')) }}">
@endpush

@push('page-style')
    <link rel="stylesheet"
        href="{{ asset(mix('css/base/plugins/forms/form-validation.css')) }}">
    <link rel="stylesheet"
        href="{{ asset(mix('css/base/plugins/forms/pickers/form-flat-pickr.css')) }}">
@endpush

@section('content')
    <h3>Users List</h3>
    <p class="mb-2">

    </p>

    {{-- Tool Bar --}}
    <div class="card">
        <div class="card-body d-flex justify-content-between">
            <div class="d-flex me-auto">
                {{-- Sort Button --}}
                <div class="btn-group me-2 flex-wrap">
                    <button class="btn btn-outline-info dropdown-toggle"
                        type="button"
                        id="sortBtn"
                        data-bs-toggle="dropdown"
                        aria-expanded="false">
                        Sort
                    </button>

                    {{-- Input Save Sort Type For Send To Server --}}
                    <input type="hidden"
                        name="sort_type"
                        value="{{ config('constants.sort.latest') }}">

                    <div class="dropdown-menu"
                        aria-labelledby="dropdownMenuButton">
                        <li>
                            <a class="dropdown-item sort-item"
                                data-sort="{{ config('constants.sort.latest') }}"
                                href="#">Latest
                            </a>
                        </li>

                        <li>
                            <a class="dropdown-item sort-item"
                                data-sort="{{ config('constants.sort.oldest') }}"
                                href="#">
                                Oldest
                            </a>
                        </li>

                        <li>
                            <a class="dropdown-item sort-item"
                                data-sort="{{ config('constants.sort.a-z') }}"
                                href="#">
                                A-Z
                            </a>
                        </li>

                        <li>
                            <a class="dropdown-item sort-item"
                                data-sort="{{ config('constants.sort.z-a') }}"
                                href="#">
                                Z-A
                            </a>
                        </li>
                    </div>
                </div>
                {{-- /Sort Button --}}

                {{-- Filter By Role --}}
                <div class="user-role width-200 me-2">
                    <select id="userRole"
                        class="form-select text-capitalize filter">
                        <option value="">Select Role</option>
                        @foreach ($roles as $role)
                            <option value="{{ $role->id }}">
                                {{ $role->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                {{-- /Filter By Role --}}

                {{-- Filter By Status --}}
                <div class="user-status width-200">
                    <select id="userStatus"
                        class="form-select text-capitalize filter">
                        <option value="">Select Status</option>
                        <option value="{{ \App\Models\User::STATUS_ACTIVE }}">
                            {{ __(\App\Models\User::STATUS_ACTIVE_NAME) }}
                        </option>
                        <option value="{{ \App\Models\User::STATUS_INACTIVE }}">
                            {{ __(\App\Models\User::STATUS_INACTIVE_NAME) }}
                        </option>
                    </select>
                </div>
                {{-- /Filter By Status --}}
            </div>

            {{-- Create Button --}}
            <div class="btn-group">
                <button class="btn btn-outline-success"
                    type="button"
                    id="createUser"
                    data-url-create="{{ route('users.create') }}"
                    data-url-store="{{ route('users.store') }}">
                    New User
                </button>
            </div>
            {{-- /Create Button --}}
        </div>
    </div>
    {{-- /Tool Bar --}}

    {{-- Table Use For Get Datatables Form Server --}}
    <table id="userTbl"
        style="display: none;"
        cellspacing="0"
        width="100%"></table>


    {{-- Container List User(Get From '#userTbl') --}}
    <div class="row my-2"
        id="userList"
        style="display: none"
        data-url-datatables="{{ route('users.datatables') }}">
    </div>


    {{-- Save User Modal --}}
    <x-modals.save-user></x-modals.save-user>

@endsection

@push('vendor-script')
    <script src="{{ asset(mix('vendors/js/tables/datatable/jquery.dataTables.min.js')) }}"></script>
    <script src="{{ asset(mix('vendors/js/tables/datatable/dataTables.bootstrap5.min.js')) }}"></script>
    <script src="{{ asset(mix('vendors/js/tables/datatable/dataTables.responsive.min.js')) }}"></script>
    <script src="{{ asset(mix('vendors/js/tables/datatable/responsive.bootstrap5.js')) }}"></script>
    <script src="{{ asset(mix('vendors/js/tables/datatable/datatables.buttons.min.js')) }}"></script>
    <script src="{{ asset(mix('vendors/js/tables/datatable/buttons.bootstrap5.min.js')) }}"></script>
    <script src="{{ asset(mix('vendors/js/tables/datatable/datatables.checkboxes.min.js')) }}"></script>
    <script src="{{ asset(mix('vendors/js/forms/select/select2.full.min.js')) }}"></script>
    <script src="{{ asset(mix('vendors/js/pickers/flatpickr/flatpickr.min.js')) }}"></script>
@endpush

@push('page-script')
    <script src="{{ asset(mix('js/scripts/pages/page-user.js')) }}"></script>
@endpush
